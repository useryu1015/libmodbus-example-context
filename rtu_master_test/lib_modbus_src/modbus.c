...
static int receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    uint8_t function = msg[0];              // 0x2B： 形参不够用。用于验证功能码类型
#endif
...
#ifdef USE_FUNCTION_0X2B
    if (function == 0x2B)
        length_to_read = MAX_MESSAGE_LENGTH;                    // change： 直接接收单次最大接收数量。
#endif
...
#ifdef USE_FUNCTION_0X2B
        offset = _MODBUS_RTU_RESPONSE_USELESS_FRAME_LEN - 2;			
        rc = rc - _MODBUS_RTU_RESPONSE_USELESS_FRAME_LEN - 2;                                   // -帧头，-校验帧
        for (i = 0; i < rc; i++) {
...
// #define _MODBUS_RTU_RESPONSE_USELESS_FRAME_LEN   8			// 响应报文，非数据帧长度

static int receive_deviceInfo(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    int rc;
    fd_set rfds;
    struct timeval tv;
    ...
    return ctx->backend->check_integrity(ctx, msg, msg_length);     // 校验CRC值 
}

/* Reads the data from a remove device and put that data into an array 
 * 负责报文的发送、接收和格式化 */
static int read_deviceInfo(modbus_t *ctx, int function, int code, int oi,
                          uint16_t *dest)
{
    int rc;
    int req_length;
    ...
    req_length = ctx->backend->build_request_basis(ctx, function, code, oi, req);       // 构建报文头
    ...
    if (rc > 0) {
        ...
        /* Display the hex code of each character received */
        if (ctx->debug) {
            printf(" recv all data[%d]:    \n",rc);    
            for (i=0; i < rc; i++)
                printf("<%.2X>", rsp[i]);          // 打印接收的数据
        }


        // rc = check_confirmation(ctx, req, rsp, rc);          // 校验数据 格式是否正确
        // if (rc == -1)
        //     return -1;
 ...
 * 功能码： 0x2B
 * p1：fd
 * p2：read dec id code（读取方式，只取： 01/02/03/04）
 * p3：objec id（要读取的对象标识）
 * p4：read buf
 * tips： 目前未兼容 tcp 模式
 */
int modbus_read_deviceInfo(modbus_t *ctx, uint8_t code, uint8_t oi, uint8_t *dest)
{
    ...
    printf("\n\n bigin once DI \n\n\n\n\n");
    status = read_io_status(ctx, _FC_READ_DEVICE_INFO,
                            code, oi, dest);
    return status;
#else
    printf(" Not define USE_FUNCTION_0X2B \n");
#endif
}