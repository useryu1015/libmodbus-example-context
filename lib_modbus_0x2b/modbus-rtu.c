...
static int _modbus_rtu_build_request_basis(modbus_t *ctx, int function, int addr, int nb, uint8_t *req) {
	...
	assert(ctx->slave != -1);
    if ((function & 0x00ff) == 0x2B) {
        req[0] = ctx->slave;
        req[1] = function & 0x00ff;
        req[2] = 0x0E;
        req[3] = addr & 0x00ff;         // Device ID code
        req[4] = nb & 0x00ff;           // Object ID

        return _MODBUS_RTU_PRESET_REQ_LENGTH_DEVICE_ID;
    } else {
		req[0] = ctx->slave;
        req[1] = function;
        req[2] = addr >> 8;
...
int _modbus_rtu_recv_msg_post(uint8_t *rsq, int rsq_length){
	return rsq_length;
}
...
static int _modbus_rtu_connect(modbus_t *ctx) {
	...
	if (ctx->debug) {
		zlog_info("Opening %s at %d bauds (%c, %d, %d)", ctx_rtu->device, ctx_rtu->baud, ctx_rtu->parity,
	...
	if (ctx_rtu->w_ser.fd == INVALID_HANDLE_VALUE) {
		zlog_debug("Can't open the device %s (%s)", ctx_rtu->device, strerror(errno));
		return -1;
	}
	...
	switch (ctx_rtu->baud) {
		...
		default:
		dcb.BaudRate = CBR_9600;
		zlog_warn("Unknown baud rate %d for %s (B9600 used)", ctx_rtu->baud, ctx_rtu->device);
	}
...
MODBUS_RTU_MAX_ADU_LENGTH, _modbus_set_slave, _modbus_rtu_build_request_basis,
		_modbus_rtu_build_response_basis, _modbus_rtu_prepare_response_tid, _modbus_rtu_send_msg_pre,
		_modbus_rtu_send, _modbus_rtu_recv, _modbus_rtu_recv_msg_post, _modbus_rtu_check_integrity,
		_modbus_rtu_pre_check_confirmation, _modbus_rtu_connect, _modbus_rtu_close, _modbus_rtu_flush, _modbus_rtu_select,
...
modbus_t* modbus_new_rtu(const char *device, int baud, char parity, int data_bit, int stop_bit) {
	...
	_modbus_init_common(ctx, _MODBUS_SERIAL);
