...
static int _modbus_tcp_init_win32(void)
{
	...
	if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
		zlog_trace("WSAStartup() returned error code %d", (unsigned int)GetLastError());
		errno = EIO;
		return -1;
	}
	return 0;
...
int _modbus_tcp_recv_msg_post(uint8_t *rsq, int rsq_length){
	return rsq_length;
}
...
int _modbus_tcp_pre_check_confirmation(modbus_t *ctx, const uint8_t *req, const uint8_t *rsp, int rsp_length) {
	...
	if (req[0] != rsp[0] || req[1] != rsp[1]) {
		if (ctx->debug) {
			zlog_trace("Invalid TID received 0x%X (not 0x%x)", (rsp[0] << 8) + rsp[1],
					(req[0] << 8) + req[1]);
		}
		errno = EMBBADDATA;
		return -1;
...
static int _modbus_tcp_connect(modbus_t *ctx) {
    ...
    zlog_info("");
	if (ctx->debug) {
		zlog_info("Connecting to %s:%d", ctx_tcp->ip, ctx_tcp->port);
	}
    zlog_info("");
...
int modbus_tcp_listen(modbus_t *ctx, int nb_connection) {
	...
	yes = 1;
	if (setsockopt(new_socket, SOL_SOCKET, SO_REUSEADDR, (char *) &yes, sizeof(yes)) == -1) {
		close(new_socket);
		return -1;
	}
	...
	if (ctx->debug) {
		...
		zlog_info("Listen to %s:%d (max_connections: %d)", buf, ctx_tcp->port, nb_connection);
	}
...
int modbus_tcp_pi_listen(modbus_t *ctx, int nb_connection) {
	...
	for (ai_ptr = ai_list; ai_ptr != NULL; ai_ptr = ai_ptr->ai_next) {
		...
		if (s < 0) {
			if (ctx->debug) {
				zlog_trace("socket = %d", s);
			}
			continue;
		} else {
			int yes = 1;
			rc = setsockopt(s, SOL_SOCKET, SO_REUSEADDR, (void *) &yes, sizeof(yes));
			if (rc != 0) {
				if (ctx->debug) {
					zlog_trace("setsockopt = %d", s);
				}
				close(s);
				continue;
			}
...
int modbus_tcp_accept(modbus_t *ctx, int *socket) {
	...
	if (ctx->debug) {
		zlog_info("The client connection from %s is accepted", inet_ntoa(addr.sin_addr));
	}
...
int modbus_tcp_pi_accept(modbus_t *ctx, int *socket) {
	...
	if (ctx->debug) {
		zlog_info("The client connection is accepted.");
	}
...
MODBUS_TCP_MAX_ADU_LENGTH, _modbus_set_slave, _modbus_tcp_build_request_basis,
		_modbus_tcp_build_response_basis, _modbus_tcp_prepare_response_tid, _modbus_tcp_send_msg_pre,
		_modbus_tcp_send, _modbus_tcp_recv, _modbus_tcp_recv_msg_post, _modbus_tcp_check_integrity,
		_modbus_tcp_pre_check_confirmation, _modbus_tcp_connect, _modbus_tcp_close, _modbus_tcp_flush,
		_modbus_tcp_select, _modbus_tcp_filter_request };
...
MODBUS_TCP_MAX_ADU_LENGTH, _modbus_set_slave, _modbus_tcp_build_request_basis,
		_modbus_tcp_build_response_basis, _modbus_tcp_prepare_response_tid, _modbus_tcp_send_msg_pre,
		_modbus_tcp_send, _modbus_tcp_recv, _modbus_tcp_recv_msg_post, _modbus_tcp_check_integrity,
		_modbus_tcp_pre_check_confirmation, _modbus_tcp_pi_connect, _modbus_tcp_close, _modbus_tcp_flush,
		_modbus_tcp_select, _modbus_tcp_filter_request };
...
modbus_t* modbus_new_tcp(const char *ip, int port) {
	...
	if (sigaction(SIGPIPE, &sa, NULL) < 0) {
		...
		zlog_trace("Coud not install SIGPIPE handler.");
		return NULL;
	}
#endif

	ctx = (modbus_t *) malloc(sizeof(modbus_t));
	_modbus_init_common(ctx, _MODBUS_TCP);
	...
	if (ret_size == 0) {
		zlog_trace("The IP string is empty");
		modbus_free(ctx);
		errno = EINVAL;
		return NULL;
	}
	...
	if (ret_size >= dest_size) {
		zlog_trace("The IP string has been truncated");
		modbus_free(ctx);
		errno = EINVAL;
		return NULL;
	}
...
modbus_t* modbus_new_tcp_pi(const char *node, const char *service) {
	...
	if (ret_size == 0) {
		zlog_trace("The node string is empty");
		modbus_free(ctx);
		errno = EINVAL;
		return NULL;
	}
	...
	if (ret_size >= dest_size) {
		zlog_trace("The node string has been truncated");
		modbus_free(ctx);
		errno = EINVAL;
		return NULL;
	}
	...
	if (ret_size == 0) {
		zlog_trace("The service string is empty");
		modbus_free(ctx);
		errno = EINVAL;
		return NULL;
	}
	...
	if (ret_size >= dest_size) {
		zlog_trace("The service string has been truncated");
		modbus_free(ctx);
		errno = EINVAL;
		return NULL;
	}
